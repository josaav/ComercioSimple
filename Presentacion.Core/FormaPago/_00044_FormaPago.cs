    ...
    public partial class _00044_FormaPago : FormBase
    {
        ...
        private void confirmarBoton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!_vieneDeVentas)
                {
                    long numeroModificado = _facturaServicio.Modificar(_facturaNueva);
                    long idCliente = _facturaNueva.ClienteId;
                    MessageBox.Show("Los datos se grabaron correctamente.");
                    RealizoVenta = true;

                    var mostrarFactura = new _000XX_VerFactura(numeroModificado, idCliente);
                ...
                {
                    // FACTURA
                    long numeroDeComprobante = _facturaServicio.Insertar(_facturaNueva);
                    long idCliente = _facturaNueva.ClienteId;
                    MessageBox.Show("Los datos se grabaron correctamente.");
                    RealizoVenta = true;

                    var mostrarFactura = new _000XX_VerFactura(numeroDeComprobante, idCliente);
                    mostrarFactura.ShowDialog();
